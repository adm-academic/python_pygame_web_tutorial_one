...
def draw_text(surf, text, size, x, y): # функция отрисовки текста на поверхности
    ...
    surf.blit(text_surface, text_rect) # блиттинг отрендеенного текста на переданную поверхность
...
class Player(pygame.sprite.Sprite): # Класс спрайта "Игрок"
    def __init__(self): # констуктор
        ...
        self.image.set_colorkey(BLACK) # устанока прозрачного цвета спрайта
        self.rect = self.image.get_rect() # получим прямоугольник игрока
        self.radius = 20 # установим радиус для игрока
        # pygame.draw.circle(self.image, RED, self.rect.center, self.radius)
        self.rect.centerx = WIDTH / 2 # изменим прямоугольник игрока
        self.rect.bottom = HEIGHT - 10 # изменим прямоугольник игрока
    ...
    def update(self): # метод обновляющий спрайт в соответсвии с игровой логикой
        ...
        if self.power >= 2 and pygame.time.get_ticks() - self.power_time > POWERUP_TIME: # обабатывает силу выстрелов
            self.power -= 1 # ????
            self.power_time = pygame.time.get_ticks() # ????
...
while running: # пока работаем...
    ...
    screen.blit(background, background_rect) # выводим фоновую катинку
